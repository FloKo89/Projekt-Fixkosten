...
class ListFrame(ttk.Frame):
    def __init__(self, container, **kwargs):
        ...
        treeview_fix_costs = ttk.Treeview(self, selectmode="browse", columns=("reciever", "sum", "debit interval"))
        treeview_fix_costs.grid(column=0, row=0, columnspan=2, pady=15)

        treeview_fix_costs.heading("#0", text="Tree Column")
        treeview_fix_costs.heading("reciever", text="Empfänger")
        treeview_fix_costs.heading("sum", text="Betrag")
        treeview_fix_costs.heading("debit interval", text="Abbuchungsintervall")
        treeview_fix_costs.column("#0", width=0, stretch=tk.NO)
        treeview_fix_costs.column("reciever", width=220)
        treeview_fix_costs.column("sum", width=115)
        treeview_fix_costs.column("debit interval", width=120)

        scrollbar_list = ttk.Scrollbar(self, orient="vertical", command=treeview_fix_costs.yview)
        scrollbar_list.grid(column=3, row=0, sticky="ns")
        treeview_fix_costs.configure(xscrollcommand=scrollbar_list.set)
